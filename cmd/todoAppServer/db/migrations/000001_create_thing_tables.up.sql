...
CREATE SCHEMA todo;
CREATE TYPE todo_status_type AS ENUM (
    'Planifié', -- This state refers to sometodo that is in the early stages of development, where the design and construction plans have been finalized, but physical construction has not yet begun.
    'En Construction', -- Sometodo is in the process of being built.
    'Utilisé', -- The todo is now being utilized for its intended purpose.
    'Abandonné', -- The todo is no longer in use, being utilized
    'Démoli'
    );
-- TO LIST THOSE ENUM VALUES:
--SELECT unnest(enum_range(NULL::todo_status_type))
/*
-- Table structure for table todo for project go-cloud-k8s-todo
-- version : 0.0.4
*/
...
(
    ...
    id                 uuid    not null
        constraint pk_todo primary key default gen_random_uuid(),
...
SELECT AddGeometryColumn('todo', 'todo', 'position', 2056, 'POINT', 2);
CREATE INDEX idx_todo_geom_gist ON todo.todo USING gist (position);
CREATE INDEX idx_todo_type_id ON todo.todo (type_id);


--
-- Table structure for table `TypetodoApp` generated from model 'TypetodoApp'
--
CREATE TABLE IF NOT EXISTS todo.type_todo
(
    id                 serial
        constraint pk_type_todo primary key,
    name               text                    not null,
    description        text,
    comment            text,
    external_id        integer,
...
alter table todo.todo
    add constraint todo_type_todo_id_fk
        foreign key (type_id) references todo.type_todo;

-- #### BEGIN OF DATA FROM GOELAND
-- imported from TypetodoApp in goeland as of 2023-07-21
INSERT INTO todo.type_todo (id, name, description, comment, external_id, table_name, inactivated, inactivated_time,
...
create index type_todo_text_search_index
    on todo.type_todo using gin (text_search);

SELECT setval('todo.type_todo_id_seq', max(id)) FROM todo.type_todo;
