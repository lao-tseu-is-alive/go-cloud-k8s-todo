...
CREATE SCHEMA todo;
CREATE TYPE todo_app_status_type AS ENUM (
    'Planifié', -- This state refers to sometodo_app that is in the early stages of development, where the design and construction plans have been finalized, but physical construction has not yet begun.
    'En Construction', -- Sometodo_app is in the process of being built.
    'Utilisé', -- The todo_app is now being utilized for its intended purpose.
    'Abandonné', -- The todo_app is no longer in use, being utilized
    'Démoli'
    );
-- TO LIST THOSE ENUM VALUES:
--SELECT unnest(enum_range(NULL::todo_app_status_type))
/*
-- Table structure for table todo_app for project go-cloud-k8s-todo
-- version : 0.0.4
*/
CREATE TABLE IF NOT EXISTS todo.todo_app
(
    ...
    id                 uuid    not null
        constraint pk_todo_app primary key default gen_random_uuid(),
    type_id            integer not null,
    name               text    not null constraint todo_unique_name	unique
...
SELECT AddGeometryColumn('todo', 'todo_app', 'position', 2056, 'POINT', 2);
CREATE INDEX idx_todo_app_geom_gist ON todo.todo_app USING gist (position);
CREATE INDEX idx_todo_app_type_id ON todo.todo_app (type_id);


--
-- Table structure for table `TypetodoApp` generated from model 'TypetodoApp'
--
CREATE TABLE IF NOT EXISTS todo.type_todo_app
(
    id                 serial
        constraint pk_type_todo_app primary key,
    name               text                    not null,
    description        text,
    comment            text,
    external_id        integer,
...
alter table todo.todo_app
    add constraint todo_app_type_todo_app_id_fk
        foreign key (type_id) references todo.type_todo_app;

-- #### BEGIN OF DATA FROM GOELAND
-- imported from TypetodoApp in goeland as of 2023-07-21
INSERT INTO todo.type_todo_app (id, name, description, comment, external_id, table_name, inactivated, inactivated_time,
...
create index type_todo_app_text_search_index
    on todo.type_todo_app using gin (text_search);

SELECT setval('todo.type_todo_app_id_seq', max(id)) FROM todo.type_todo_app;
